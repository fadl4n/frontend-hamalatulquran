...
import 'package:frontend_hamalatulquran/pages/lainnya_page.dart';
...
class MenuGrid extends StatelessWidget {
  ...
  const MenuGrid({super.key, required this.isPengajar, this.santri});
  ...
  void _navigateToPage(BuildContext context, String label) {
    debugPrint(
        "🚀 Navigating to $label, isPengajar: $isPengajar, santri: ${santri != null}");

    if (!isPengajar && santri != null) {
      debugPrint("Role santri detected. Handling navigation for santri.");
      switch (label) {
        case "Target Hafalan":
          debugPrint("Navigating to TargetHafalanSantri");
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (_) => TargetHafalanSantri(santri: santri!),
            ),
          );
          return;
        case "Evaluasi Muroja'ah":
          debugPrint("Navigating to EvaluasiListSurat");
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (_) => EvaluasiListSurat(santri: santri!),
            ),
          );
          return;
      }
    }
    debugPrint("Navigating to default route");
